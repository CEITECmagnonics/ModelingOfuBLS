...
[xi, yi, Exi, Eyi, Ezi] = focalFields(lambda, NA, 0, 2, FocalLength, pi/dqx(2), 2*N-1);
...
[ff, f00fkx] = SpinWaveGreen(qx+0.0001, 0.05, hNiFe, length(qx), 80, 0, -1000e9);
...
legendStrings = "f = " + string(ff(jj));
legend(legendStrings)

Export(:,1) = Xi(:)*1e6; 
Export(:,2) = Yi(:)*1e6; 
Export(:,3) = Xi(:)*1e6+Ex(:)./max(abs(Ex(:)))/10; 
Export(:,4) = Yi(:)*1e6+Ey(:)./max(abs(Ey(:)))/10; 

Export = real(Export);
E2 = Ex.*conj(Ex) + Ey.*conj(Ey);
