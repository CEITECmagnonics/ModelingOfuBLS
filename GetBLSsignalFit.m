% 19e-3*GetBLSsignalFit(100*1e-9, 26e-3, 0, mu*1e9, x)
function BLSsignal = GetBLSsignalFit(hNiFe, Bext, n, mu, f)
% clearvars;
% clc;
% clf;
% Used constants
...
[ff, f00fkx] = SpinWaveGreen(qx+0.0001, Bext, hNiFe, length(qx), 35, n, mu);
...
%     PSFFilter=abs(Exi);
    ...
    BLSsignal(i)=dxi*dyi*sum(Ey(:).*conj(Ey(:)));
    
    ExS(i)=dxi*dyi*sum(Ex(:));
    EyS(i)=dxi*dyi*sum(Ey(:));
end

 BLSsignal = double(interp1(ff, BLSsignal, f, 'linear', 0));
 BLSsignal = BLSsignal./max(BLSsignal(:));
